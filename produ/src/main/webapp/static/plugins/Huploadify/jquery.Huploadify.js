(function ($) {
    $.fn.Huploadify = function (opts) {
        ...
        this.each(function () {
            ...
            var inputStr = '<input id="select_btn_' + instanceNumber + '" class="selectbtn" style="display:none;" type="file" name="' + _this.attr('id') + '"';
            ...
            var fileObj = {
                ...
                onSelect: function (files) {
                    ...
                    // 全部上传
                    $('.uploadallbtn').off('click').on('click', this.funUploadAllFile); // 绑定按钮点击事件
                    _this.off('upload').on('upload', this.funUploadAllFile); // 添加upload事件
                ...
                funUploadAllFile: function () {
                    ...
                    if (xhr.upload) {
                            ...
                            if (xhr.readyState == 4) {
                                if (xhr.status == 200) {
                                    ...
                                    for (var i = 0; i < fileObj.fileFilter.length; i++) {
                                        var thisfile = _this.find('#fileupload_' + instanceNumber + '_' + fileObj.fileFilter[i].index);
                                        thisfile.find('.uploadify-progress-bar').css('width', '100%');
                                        option.showUploadedSize && thisfile.find('.uploadedsize').text(thisfile.find('.totalsize').text());
                                        option.showUploadedPercent && thisfile.find('.up_percent').text('100%');
                                    }
                                    ...
                                    // 上传完成回调事件
                                    option.onUploadSuccess && option.onUploadSuccess(fileObj.fileFilter, xhr.responseText);

                                    // 在指定的间隔时间后删掉进度条
                                    setTimeout(function () {
                                        for (var i = 0; i < fileObj.fileFilter.length; i++) {
                                            _this.find('#fileupload_' + instanceNumber + '_' + fileObj.fileFilter[i].index).fadeOut();
                                            fileObj.funDeleteFile(fileObj.fileFilter[i].index); // 从队列中删除文件
                                        }
                                    }, option.removeTimeout);
                                } else {
                                    option.onUploadError && option.onUploadError(fileObj.fileFilter, xhr.responseText);
                                }
                                option.onUploadComplete && option.onUploadComplete(fileObj.fileFilter, xhr.responseText);
                                //清除文件选择框中的已有值
                                fileObj.fileInput.val('');
                            }
                        };
                        ...
                        option.onUploadStart && option.onUploadStart();
                        // 开始上传
                        xhr.open(option.method, fileObj.url, true);
                        xhr.setRequestHeader("X-Requested-With", "XMLHttpRequest");
                        var fd = new FormData();
                        for (var i = 0; i < fileObj.fileFilter.length; i++) {
                            fd.append(option.fileObjName, fileObj.fileFilter[i]);
                        }
                        if (option.formData) {
                            if (typeof(option.formData) === 'function') {
